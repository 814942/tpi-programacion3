...
public interface UsuarioRepository extends BaseRepository<Usuario, Long> {
    ...
    @Query("SELECT u FROM Usuario u WHERE u.eliminado = false AND (LOWER(u.nombre) LIKE LOWER(CONCAT('%', :search, '%')) OR LOWER(u.apellido) LIKE LOWER(CONCAT('%', :search, '%')) OR LOWER(u.email) LIKE LOWER(CONCAT('%', :search, '%')))")
